...
async def on_message(event, actions, Manager, Segments, reminder):
    try:
        ...
        await actions.send(
            group_id=event.group_id,
            message=Manager.Message(Segments.Text("呜呜…点歌功能好像出了点小问题呢(╥﹏╥) 主人稍等一下，简儿马上修好它～"))
        )
        return True
            ...
            async with session.get(url, timeout=30) as response:
                if response.status == 200:
                    ...
                    with open(temp_file, 'wb') as f:
                        while True:
                            ...
                            if file_size > 50 * 1024 * 1024:
                                ...
                                await actions.send(
                                    group_id=event.group_id,
                                    message=Manager.Message(Segments.Text("⚠️ 啊呀！下载过程中发现这首歌太大啦(´•̥ ̯ •̥`) 超过50MB了呢，发不了音频文件…但是宝宝可以点链接听哦！"))
                                )
                                return
                    ...
                    if os.path.getsize(temp_file) > 50 * 1024 * 1024:
                        ...
                        await actions.send(
                            group_id=event.group_id,
                            message=Manager.Message(Segments.Text("⚠️ 这首歌真的太大啦(；´Д｀) 超过50MB限制了呢，简儿发不了音频文件呢…"))
                        )
                        return
